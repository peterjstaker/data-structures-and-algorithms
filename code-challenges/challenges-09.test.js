...
function transformToLis(obj){
  ...
  const liArr = objArr.reduce((answerSoFar, value) => {
    answerSoFar.push(`<li>${value[0]}: ${value[1]}</li>`);
    return answerSoFar;
  }, []);
  return liArr;
}
...
const addValues = (arr) => {
  const sum = arr.reduce((answerSoFar, value) => {
    answerSoFar+= value;
    return answerSoFar;
  }, 0);
  return sum;
...
const addPurchases = (arr) => {
  const totalPurchase = arr.reduce((answerSoFar, value) => {
...
const countNumberOfElements = (arr) => {
  const countElements = arr.reduce((answerSoFar) => {
    answerSoFar += 1;
    return answerSoFar;
  }, 0);
...
const reversedString = (str) => {
  ...
  const newStr = strArr.reduce((answerSoFar, value) => {
...
const countNumberOfChildren = (arr) => {
  const total = arr.reduce((answerSoFar, value) => {
...
const calculateAverage = (arr) => {
  const countAndSum = arr.reduce((answerSoFar, value) => {
    answerSoFar.count += 1;
    answerSoFar.sum += value;
    return answerSoFar;
  }, { count: 0, sum: 0 });
  const avg = countAndSum.sum / countAndSum.count;
  return avg;
};
...
const countPrimeNumbers = (arr) => {
  const countPrime = arr.reduce((answerSoFar, value) => {
    if(isPrime(value)){
      answerSoFar += 1;
    }
    return answerSoFar;
...
const extractStat = (statName, arr) => {
  const statObj = arr.reduce((answerSoFar, value) => {
...
const extractChildren = (arr) => {
  const filteredArr = arr.filter( value => value.name.includes('a'));
  const childrenArr = filteredArr.reduce((answerSoFar, value) => {
    if(value.children !== undefined){
      value.children.forEach(child => answerSoFar.push(child));
    }
    return answerSoFar;
  }, []);
  return childrenArr;
