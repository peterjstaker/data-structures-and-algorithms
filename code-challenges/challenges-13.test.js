...
const standardizePhoneNumbers = (arr) => arr.map(string => string.split('').filter(char => /\d/.test(char)).join(''));
...
const unenrollBrook = (arr) => arr.map(array => array.filter(person => !person.includes('Brook')));
