...
const sortByChildren = (charArray) => charArray.sort((a,b) => a.children.length === b.children.length ? (a.house < b.house ? -1 : 1) : a.children.length < b.children.length ? -1 : 1);
...
const updateNumbers = (obj) => Object.entries(obj).map(entry => entry.join(': '));
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(person => {
    const newArr = Object.values(person);
    if(newArr[0] === character && newArr[2] !== []){
      hasChildren = true;
    }});
  return hasChildren;
};
