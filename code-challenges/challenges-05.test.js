...
const sortStarWarsCharacters = (starWarsArr) => starWarsArr.sort( (a,b) => parseInt(a.height) > parseInt(b.height) ? -1 : 1);
...
const howMuchPencil = (str) => {
  ...
  for(let i = 0; i < str.length; i++){
    newStr = newStr.split('').slice(1).join('');
    result.push(newStr);
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  if(numberOfCharacters <= 0){
    ...
    str.split('').splice(str.length-numberOfCharacters).join('');
  }
};
...
const totalSumCSV = (str) => {
  let total = 0;
  str.split(',').forEach(value => total += parseInt(value));
  return total;
};
...
const removeVowels = (str) => {
  ...
  newStrArr.forEach(letter => {
    if(/[aeiou]/.test(letter)){
      newStrArr.splice(newStrArr.indexOf(letter), 1);
    }
  });
...
const extractVowels = (str) => {
  ...
  localArr.push(newStr);
  localArr.push(vowelStr);
  return localArr;
};
